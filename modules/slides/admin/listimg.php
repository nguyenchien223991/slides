 ...
 * @Createdate Fri, 20 Jun 2014 07:20:08 GMT
 */
...
if ( $nv_Request->isset_request( 'submit', 'post' ) )
{
	...
	if( empty( $error ) )
	{
		try
		{
			...
			{
				$stmt = $db->prepare( 'UPDATE ' . NV_PREFIXLANG . '_' . $module_data . '_picture SET name = :name, path = :path WHERE pictureid=' . $row['pictureid'] );
			}
...
if ( ! $nv_Request->isset_request( 'id', 'post,get' ) )
{
	...
	if( ! empty( $q ) )
	{
		$db->where( 'name LIKE :q_name OR path LIKE :q_path' );
	}
...
$page_title = $lang_module['listimg'];
