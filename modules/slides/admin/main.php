 ...
 * @Createdate Fri, 20 Jun 2014 07:22:58 GMT
 */
...
if ( $nv_Request->isset_request( 'submit', 'post' ) )
{
	...
	if( empty( $error ) )
	{
		try
		{
			...
			{
				$stmt = $db->prepare( 'UPDATE ' . NV_PREFIXLANG . '_' . $module_data . '_slide SET name = :name, active = :active WHERE slideid=' . $row['slideid'] );
			}
...
if ( ! $nv_Request->isset_request( 'id', 'post,get' ) )
{
	...
	if( ! empty( $q ) )
	{
		$db->where( 'name LIKE :q_name OR active LIKE :q_active' );
	}
